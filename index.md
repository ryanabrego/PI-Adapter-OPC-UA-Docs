---
uid: index
---

# OSIsoft Adapter for OPC UA

=======

- [OSIsoft Adapter for OPC UA overview](xref:OSIsoftAdapterForOPCUAOverview)
  - [OSIsoft Adapter for OPC UA operational overview](xref:OSIsoftAdapterForOPCUAOperationalOverview)
  - [Supported features](xref:SupportedFeatures) 
    - [OSIsoft Adapter for OPC UA supported features](xref:OSIsoftAdapterForOPCUASupportedFeatures)
  - [OSIsoft Adapter for OPC UA principles of operation](xref:OSIsoftAdapterForOPCUAPrinciplesOfOperation)
    - [Data buffering](xref:DataBuffering)
      - [Adapter to OCS buffering](xref:AdapterToOCSBuffering)
      - [Adapter to PI Server buffering](xref:AdapterToPIServerBuffering)
- [Prepare for adapter installation](xref:PrepareForAdapterInstallation)
  - [Software and hardware requirements](xref:SoftwareAndHardwareRequirements)
  - [Download OSIsoft Adapter](xref:DownloadOSIsoftAdapter)
- [Install the adapter](xref:InstallTheAdapter)
  - [Uninstall the adapter](xref:UninstallTheAdapter)
- [Configure OSIsoft Adapter for OPC UA](xref:ConfigureOSIsoftAdapterForOPCUA)
  - [Configuration schemas](xref:ConfigurationSchemas)
    - [Data source](xref:DataSourceOPCUA)
    - [Data selection](xref:DataSelectionOPCUA)
    - [Message logging](xref:MessageLogging)
    - [Endpoints](xref:Endpoints)
  - [Configure data source](xref:ConfigureDataSourceOPCUA)
  - [Configure data selection](xref:ConfigureDataSelectionOPCUA)
  - [Configure message logging](xref:ConfigureMessageLogging)
  - [Configure endpoints](xref:ConfigureEndpoints)
- [Run OSIsoft Adapter for OPC UA](xref:RunOSIsoftAdapterForOPCUA)
- [Health](xref:Health)
  - [Device status](xref:DeviceStatus)
  - [Next health message expected](xref:NextHealthMessageExpected)
- [Diagnostics](xref:Diagnostics)
    - [OSIsoft Adapter diagnostics](xref:OSIsoftAdapterDiagnostics)
    - [OSIsoft Adapter for OPC UA diagnostics](xref:OSIsoftAdapterForOPCUADiagnostics)
- [Troubleshooting](xref:Troubleshooting)
  - [Logging](xref:TroubleshootingLogging)
  - [Data buffering](xref:TroubleshootingDataBuffering)
  
